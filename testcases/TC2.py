...
url = 'https://dev.blog.airbridge.io/sdk-qa/'
...
def test_instagram_login(driver):
    ...
    try:
        ...
        edit_profile_page.go_back() #뒤로가기 클릭
        edit_profile_page.go_back() #뒤로가기 클릭
        
        profile_page.click_link() #링크 클릭
        time.sleep(2)
        profile_page.click_link_bottom(url) #프로필에 저장된 링크 2개 이상일 때 url 해당하는 링크 클릭 

        time.sleep(3)
        external_page.click_playstore() #Playstore 클릭
        
        external_page.click_continue() #계속하기 클릭
        
        #설치된 상태 > 앱 실행 후 PASS
        #미설치된 상태 > 앱 스토어로 이동 후 FAIL
        time.sleep(5)
        external_page.is_app_installed()
        time.sleep(3)
